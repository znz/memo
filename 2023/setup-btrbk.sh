...
Dpkg::Pre-Invoke  {"if [ -x /usr/bin/btrbk ]; then /usr/bin/btrbk --preserve run /mnt/btr_pool/@; fi"; };
DPkg::Post-Invoke {"if [ -x /usr/bin/btrbk ]; then /usr/bin/btrbk --preserve run /mnt/btr_pool/@; fi"; };
EOF
time limactl shell $name sudo systemctl enable --now btrbk.timer
time limactl shell $name sudo etckeeper commit "Setup btrbk"
time limactl shell $name sudo systemctl start btrbk.service
